...
class FPNVAE(nn.Module):
    def __init__(self, input_dim, rd_dim, beta=1.0):
        super(FPNVAE, self).__init__()
        self.input_dim = input_dim
        self.rd_dim = rd_dim
        self.beta = beta

        # Encoder: input_dim → rd_dim*2 → (mu, logvar)
        self.fc1 = nn.Linear(input_dim, rd_dim * 1)
        self.fc_mu = nn.Linear(rd_dim * 1, rd_dim)
        self.fc_logvar = nn.Linear(rd_dim * 1, rd_dim)

        # Decoder: rd_dim → rd_dim*2 → input_dim
        self.decoder = nn.Sequential(
            nn.Linear(rd_dim, rd_dim * 1),
            nn.ReLU(inplace=True),
            nn.Linear(rd_dim * 1, input_dim),
        )

        self._weight_init()

    def _weight_init(self):
        # 使用一致的初始化方式（避免改变现有 pipeline）
    ...
    def compute_reconstruction_loss(self, x):
        """
        输入 x 为 FPN feature map: B,C,H,W
        我们与 AE 一样做 adaptive_avg_pool2d → (B,C)
        保持完全兼容你的原有 AE 接口
        """
...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.use_rd = use_rd
        if self.use_rd:
            self.rd_modules = nn.ModuleDict()
            self.rd_records = {}
            backbone_channels = {'p2': 256, 'p3': 256, 'p4': 256, 'p5': 256, 'p6': 256}
            # 只使用 P5
            self.p5_key = "p5" if "p5" in backbone_channels else "P5"
            # VAE
            self.rd_modules[self.p5_key] = FPNVAE(
                input_dim=backbone_channels[self.p5_key],
                rd_dim=rd_dim,
                beta=1.0
            )
            # # AE
            # self.rd_modules[self.p5_key] = FPNAE(
            #     input_dim=backbone_channels[self.p5_key],
            #     rd_dim=rd_dim,
            #     beta=1.0
            # )

            self.rd_records[self.p5_key] = Records(max_len=rd_buffer_size)

        # 动态阈值
        self.z_hist = Records(max_len=500)   
        self.z_k = -1
        # # -------- tau logging --------
        # self.tau_history = []        # 用于保存每一步的 tau 
        # self.tau_save_path = "/root/autodl-fs/CTTA/figure/Swin_tau_records.txt"# 外部指定保存路径
        # self.batch_counter = 0  # 用于控制每200个batch保存一次
    ...
    def from_config(cls, cfg):
        ...
        return {
            ...
            "use_rd": cfg.TEST.ADAPTATION.USE_RD,
            "rd_dim": cfg.TEST.ADAPTATION.RD_DIM,
            "rd_buffer_size": cfg.TEST.ADAPTATION.RD_BUFFER_SIZE,
        }
...
# ________________________________________________________VAE
        avg_z_score = 0.0
        tau = -1

        if self.use_rd:
            p5 = self.p5_key
            cur_feats = features[p5]                # [B,C,H,W]
            ...
            # 遍历 batch
            for cur_feat in cur_feats:
                cur_rd_loss, cur_vae_kl_loss = self.rd_modules[p5].compute_reconstruction_loss(cur_feat)

                level_rd_loss += cur_rd_loss.item()
                self.rd_records[p5].add_record(torch.tensor([cur_rd_loss.item()]))

                rd_loss += cur_rd_loss
                vae_kl_loss += cur_vae_kl_loss

            # batch 平均
            level_rd_loss /= cur_feats.shape[0]
            rd_loss = rd_loss / cur_feats.shape[0]

            adapt_loss["rd_loss"] = 0.5*rd_loss
            adapt_loss["vae_kl_loss"] = 0.5*vae_kl_loss

            # ---- z-score：只使用 P5 ----
            if self.rd_records[p5].length > 2:
                mean = self.rd_records[p5].mean
                std  = self.rd_records[p5].stddev

                avg_z_score = abs((level_rd_loss - mean) / std) if std > 0 else 0
            else:
                avg_z_score = 0.0

            # ---- 动态阈值 τ（只基于 P5 的 z-score） ----
            self.z_hist.add_record(torch.tensor([avg_z_score]))
            z_mean = self.z_hist.mean
            z_std  = self.z_hist.stddev
            tau = z_mean + self.z_k * z_std
            # print(z_mean, z_std , tau)
            # # -------- record tau --------
            # self.tau_history.append(float(tau))
            # self.tau_history.append(float(tau))  # 每次记录 tau
            # # 每200个batch保存一次 tau
            # self.batch_counter += 1
            # if self.batch_counter % 200 == 0:
            #     self.save_tau_history()
...
# ______________________________________________________
        ...
        if do_postprocess:
            ...
            return GeneralizedRCNN._postprocess(results, batched_inputs, images.image_sizes), adapt_loss, feature_sim, avg_z_score, tau
        return results, adapt_loss, feature_sim, avg_z_score, tau

    def save_tau_history(self):
        if self.tau_save_path is not None:
            with open(self.tau_save_path, "a") as f:  # "a"模式可以追加记录
                for i, v in enumerate(self.tau_history):
                    f.write(f"{i}\t{v}\n")
            print(f"[INFO] Tau values saved to: {self.tau_save_path}")
            self.tau_history = []  # 清空 tau 记录，准备保存下一个 batch 的数据
