...
class DefaultTrainer(TrainerBase):
    ...
    #     self.storst = 0.1#stochastic restore probablity初始化为0.1
    # def sto_rst(self): # 随机恢复操作
    #     if True:
    #         # print('+++++++++++++++++++++++++++++++1234')
    #         for nm, m in self.checkpointer.model.backbone.bottom_up.named_modules():#模块名称nm和模块实例m
    #             if nm in ['res4','res5']:
    #                 for npp, p in m.named_parameters():#参数名称npp和对应参数张量p
    #                     if npp in ['weight', 'bias'] and p.requires_grad:#筛选出对应参数名和
    #                         # 生成随机掩码，根据概率决定是否恢复参数
    #                         mask = (torch.rand(p.shape) < self.storst).float().cuda()
    #                         with torch.no_grad():
    #                             state_dict = self.checkpointer.model.state_dict()
    #                             p.data = state_dict[f"{nm}.{npp}"] * mask + p * (1. - mask)
...
#2025.6.23添加了inference_on_dataset_online_adaption可视化
    @classmethod
    def test_continual_domain(cls, cfg, wandb=None):
        ...
        if 'coco' in dataset_name:
            test_domain = ['gaussian_noise','defocus_blur','snow','contrast'] if cfg.TEST.ADAPTATION.SELECT_DOMAIN else get_corruption_names()[:]
            # test_domain = ['gaussian_noise','shot_noise', 'impulse_noise','defocus_blur','glass_blur','motion_blur','zoom_blur','gaussian_noise','snow','frost','fog','brightness','defocus_blur','contrast','elastic_transform','pixelate','jpeg_compression','snow'] if cfg.TEST.ADAPTATION.SELECT_DOMAIN else get_corruption_names()[:]
        else:       # kitti
        ...
        for d_idx, corrupt in enumerate(test_domain):
            ...
            if cfg.TEST.ONLINE_ADAPTATION:
                # domain_visualize_dir = os.path.join(visualize_root, corrupt)
                ...
                # results_i, loss_ema99, loss_ema95, loss_ema90, is_used, total_compute_time = inference_on_dataset_online_adaptation(cfg, model, data_loader, optimizer, evaluator, d_idx, wandb, teacher_model=teacher_model, val_data_loader=None, val_evaluator=None, loss_ema99=loss_ema99, loss_ema95=loss_ema95, loss_ema90=loss_ema90, is_used=is_used, domain_name=corrupt, visualize_dir=os.path.join(cfg.OUTPUT_DIR, corrupt))
