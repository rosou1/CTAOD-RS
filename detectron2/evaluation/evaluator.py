...
#2025.6.23添加了可视化
def inference_on_dataset_online_adaptation(cfg, model, data_loader, optimizer, evaluator, d_idx, wandb, teacher_model=None, val_data_loader=None, val_evaluator=None, loss_ema99=0, loss_ema95=0, loss_ema90=0, is_used=0, domain_name=None, visualize_dir=None):
    """
    Run model on the data_loader and evaluate the metrics with evaluator.
    ...
    with EventStorage() as storage:
        for idx, inputs in enumerate(data_loader):
            ...
            if cur_used or (not cur_used and pause_iter % cfg.TEST.ADAPTATION.SKIP_PERIOD == 0):
                if cfg.TEST.ADAPTATION.TYPE == "mean-teacher":
                    ...
                    outputs, losses, feature_sim, z_score, tau = model(inputs)
